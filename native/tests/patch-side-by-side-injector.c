...
int mainCRTStartup(void) {
    ...
    uint32_t count=0;
    DWORD attempt;
    RECT left_rect={0,0,0,0}, right_rect={0,0,0,0};
    for(attempt=0;attempt<480u && !top;++attempt) {
        top=FindWindowA((const char*)0,"Spec Win32 v054 Patch Side-by-Side Smoke");
        if(!top) Sleep(25u);
    }
    if(!top) return 2;
    ...
    write_text("PATCH-SBS-INJECTOR-TWO-RICHEDIT-PASS\r\n");
    ...
    write_text("PATCH-SBS-INJECTOR-READONLY-PASS\r\n");
    if(SendMessageW(rich[0],EM_GETLINECOUNT_VALUE,0,0)<40 || SendMessageW(rich[1],EM_GETLINECOUNT_VALUE,0,0)<40) return 5;
    SendMessageW(rich[0],EM_GETRECT_VALUE,0,(LPARAM)(uintptr_t)&left_rect);
    SendMessageW(rich[1],EM_GETRECT_VALUE,0,(LPARAM)(uintptr_t)&right_rect);
    if(left_rect.left<36 || right_rect.left<36) return 6;
    write_text("PATCH-SBS-INJECTOR-LINENUMBERS-PASS\r\n");
    SendMessageW(rich[0],WM_VSCROLL_VALUE,SB_PAGEDOWN_VALUE,0);
    SendMessageW(rich[0],WM_VSCROLL_VALUE,SB_PAGEDOWN_VALUE,0);
    if(!wait_synced(rich[0],rich[1],1)) return 7;
    write_text("PATCH-SBS-INJECTOR-LEFT-TO-RIGHT-SYNC-PASS\r\n");
    SendMessageW(rich[1],WM_VSCROLL_VALUE,SB_TOP_VALUE,0);
    if(!wait_synced(rich[0],rich[1],0)) return 8;
    if(SendMessageW(rich[0],EM_GETFIRSTVISIBLELINE_VALUE,0,0)!=0) return 8;
    write_text("PATCH-SBS-INJECTOR-RIGHT-TO-LEFT-SYNC-PASS\r\n");
    Sleep(300u);
    return 0;
}
