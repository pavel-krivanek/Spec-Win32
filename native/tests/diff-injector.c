...
int mainCRTStartup(void) {
    ...
    for(attempt=0;attempt<480u && !top;++attempt) {
        top=FindWindowA((const char*)0,"Spec Win32 v055 Diff Smoke");
        if(!top) Sleep(25u);
    }
    if(!top) return 2;
    ...
    write_text("DIFF-INJECTOR-TWO-RICHEDIT-PASS\r\n");
    ...
    write_text("DIFF-INJECTOR-READONLY-PASS\r\n");
    ...
    SendMessageW(rich[0],WM_VSCROLL_VALUE,SB_PAGEDOWN_VALUE,0);
    if(!wait_synced(rich[0],rich[1],1)) return 6;
    write_text("DIFF-INJECTOR-LEFT-TO-RIGHT-SYNC-PASS\r\n");
    SendMessageW(rich[1],WM_VSCROLL_VALUE,SB_TOP_VALUE,0);
    if(!wait_synced(rich[0],rich[1],0)) return 7;
    if(SendMessageW(rich[0],EM_GETFIRSTVISIBLELINE_VALUE,0,0)!=0) return 7;
    write_text("DIFF-INJECTOR-RIGHT-TO-LEFT-SYNC-PASS\r\n");
    Sleep(300u);
    return 0;
}
